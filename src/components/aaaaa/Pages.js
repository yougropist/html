                      ...
                      {console.log(this.state.groupesPost, this.state.groupeSelected, 'here')}
                        ...
                        <div className="container-ajout create-page">
                          ...
                          <div style={{display: 'flex', justifyContent: 'center', width: '100%'}}>
                            <select style={{marginRight: '20px'}} ref='pageSelect' onChange= {(e) => this.setState({pageSelected: e.target.options[e.target.selectedIndex].id}, () => this.getGroupesByPage(this.state.pageSelected), this.getPost(this.state.pageSelected))}>
                          ...
                          switch(true) {
                            case el.type === 'input':
                              return  <li>
                                        <p>{el.title}</p>
                                        <div>
                                          <input  ref={`input${index}-${i}`} defaultValue={elem[el.content]} disabled={this.state.update.index===index&&this.state.update.i===i ?  false:true} type='text' />
                                          <button className={this.state.update.index===index&&this.state.update.i===i ? 'valide':''} onClick={(e)=> {this.setState({update: index===this.state.update.index&&i===this.state.update.i?{index:'',i:''}:{index:index,i:i}}); index===this.state.update.index&&i===this.state.update.i&& this.updateInput(elem.id,index,i)}}>{this.state.update.index===index&&this.state.update.i===i ? 'Valider':'Modifier'}</button>
                                        </div>
                                      </li>
